...
- No email or username format validation beyond uniqueness (consistent with instructor registration)
...
- Create user (`create_student_user`), create session (`create_student_session` with `expires_at = now + 30 days` as a string `"%Y-%m-%d %H:%M:%S"`), set `student_session_token` cookie (httponly, samesite=lax, max_age=30×24×3600)
- Return `{ id, username }`
...
- Clear `student_session_token` cookie (set to empty, max_age=0)
- Return `{ ok: true }`
...
16. Expired session token → `GET /api/student/auth/me` returns 401 and does not slide the window. To manufacture an expired session: call `create_student_session(token, student_id, "2000-01-01 00:00:00")` directly (bypassing the normal route), then call `/me` with that token and verify both the 401 response and that the `expires_at` in the DB is still `"2000-01-01 00:00:00"` (query via `get_student_session` returning None confirms expiry; verify the raw DB row via a direct `db.execute` to confirm `expires_at` was not updated).
